...
"""
Mirrored from `backend/services/validation.py` so the worker service has no dependency on the
backend package (independent deployability). Keep these two files in sync manually, or extract
to a shared library if this grows further. See `backend/services/validation.py` for full docs.
"""
...
def clean_policies(df: pd.DataFrame, known_customer_ids: set[str]) -> tuple[pd.DataFrame, list[str]]:
    ...
    df["state"] = df["state"].str.upper()
    df["policy_issue_date_parsed"] = pd.to_datetime(df["policy_issue_date"].astype(str).str.strip(), errors="coerce").dt.date
